<?php
	class sheet_receiver extends BaseController{
		private $objMo = null;
		function __construct($controller, $method){
				parent::__construct($controller, $method, 3);
				$this->model("Basedata_m");
				$this->objMo = new Basedata_m(3);
		}
		...
		private function makeColModel(){
				$arrFields = $this->objMo->getFields(3);
		...
		public function getList(){
			...
			//	print_r($arrFields);
				...
				$page = $_GET["page"];   //第几页
				$start = ($page - 1)*$rows;
				$arrData = $this->objMo->getData('3', array('start'=>$start,'limit'=>$rows));
				$arrAllData = $this->objMo->getData('3');
