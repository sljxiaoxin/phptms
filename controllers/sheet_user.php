<?php
	class sheet_user extends BaseController{
		...
		private function makeColModel(){
				$arrFields = $this->objMo->getFields($this->intTablePK);
		...
		public function getList(){
				...
				$page = $_GET["page"];   //第几页
				$start = ($page - 1)*$rows;
				$arrData = $this->objMo->getData($this->intTablePK, array('start'=>$start,'limit'=>$rows));
				$arrAllData = $this->objMo->getData($this->intTablePK);
		...
		public function save(){
				...
				foreach($arrFieldsInfo as $key=>$val){
						if(isset($_POST[$key])){
								...
								if($val['intType'] == '5' || $val['intType'] == '17' || $val['intType'] == '18'){
